...
//                $temp['branch_bank_short_name'] = preg_replace("/^" . $this->newBanks[$bankCode] . "/", '', $bank->branch_bank_name, 1);
                $temp['branch_bank_short_name'] = preg_replace("/股份有限公司/", '', $bank->branch_bank_name, 1);
//                $temp['branch_bank_short_name'] = $bank->branch_bank_name;
    ...
    public function getBanksV2(Request $request, $bankCode, $province, $keyword, $page = 1){
        $openid = $request->wx_openid;
        ...
        if($banks){
            ...
            foreach ($banks as $bank){
                $temp = [];
                $temp['name'] = $bank->branch_bank_name;
                $temp['address'] = $bank->branch_bank_name;
                $temp['code'] = $bank->branch_bank_code;
                $temp['id'] = $bank->id;
                if(isset($returnData['collects']) && in_array($bank->branch_bank_code, $returnData['collects'])){
                    $temp['is_collect'] = 1;
                }else{
    ...
    public function getBannerImgs(){
        ...
        $songs = ['该乐观起来，这世界有四季与远方还有火锅与理想',
            '无边落木萧萧下，不尽长江滚滚来', '远上寒山石径斜，白云深处有人家',
